...
public class PopularTVDataSource extends PageKeyedDataSource<Integer, Result> {

    private APIInterface apiInterface;

    public PopularTVDataSource() {
    ...
    public void loadInitial(@NonNull PageKeyedDataSource.LoadInitialParams<Integer> params, @NonNull LoadInitialCallback<Integer, Result> callback) {
        ...
        apiInterface.popularTV(BuildConfig.My_Token, 1).enqueue(new Callback<TVResponse>() {
            @Override
            public void onResponse(Call<TVResponse> call, Response<TVResponse> response) {
                Log.d("fragment", "onChanged: data " + response.body().getResults().size());
    ...
    public void loadBefore(@NonNull LoadParams<Integer> params, @NonNull LoadCallback<Integer, Result> callback) {
        apiInterface.popularTV(BuildConfig.My_Token, params.key).enqueue(new Callback<TVResponse>() {
            @Override
            public void onResponse(Call<TVResponse> call, Response<TVResponse> response) {
                if(response.isSuccessful()){
                    List<Result> tv;
